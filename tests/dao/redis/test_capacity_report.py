...
def test_get_rank(readings, capacity_report_dao):
    for reading in readings:
        capacity_report_dao.update(reading)
        print(f"reading: {reading} is added")
    #print(f"site 0's ranking is actually {capacity_report_dao.get_rank(readings[0].site_id)}")
    #print(f"site 7's ranking is actually {capacity_report_dao.get_rank('7')}")
