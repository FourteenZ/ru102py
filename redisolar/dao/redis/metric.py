...
class MetricDaoRedis(MetricDaoBase, RedisDaoBase):
    ...
    def insert_metric(self, site_id: int, value: float, unit: MetricUnit,
        ...
        # so the minute_of_day is the score of the ZSet?
        # with the "value" being the "element"
        # and metric_key is key of ZSet        

        # this function is the reverse of _get_measurements_for_date, as the video pointed out
        # and in that function, it pointed out
        # `zrevrange()` returns (member, score) tuples, and within these
        # tuples, "member" is a string of the form [measurement]:[minute].
        # The MeasurementMinute class abstracts this for us.        
        # which means the "member" needs to be [measurement]:[minute]

        # i think my problem is, i don't know which one of these functions will
        # put it in that format??

        # there is probably a better way to do this formating of string like this??
        # MeasurementSchema
        # I think i need to create a Measurement obj, populate it, then flatten it ?
        #m=Measurement(site_id, value, unit, time)
        #pipeline.zadd(metric_key, minute_of_day, f"[{value}]:[{minute_of_day}]")

        #member = f"[{MeasurementSchema().load(m)}]: [{minute_of_day}]"
        #pipeline.zadd(metric_key, mapping={member :  minute_of_day})
